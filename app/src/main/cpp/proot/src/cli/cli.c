...
#define DETAIL(a) if (detailed) a
	...
	DETAIL(NOTE_LOGD("%s %s: %s.\n\n", cli->name, cli->version, cli->subtitle));
	NOTE_LOGD("Usage:\n  %s\n", cli->synopsis);
	DETAIL(NOTE_LOGD("\n"));

	options = cli->options;
	for (i = 0; options[i].className != NULL; i++) {
		for (j = 0; ; j++) {
			...
			if (strcmp(options[i].className, current_class) != 0) {
				current_class = options[i].className;
				NOTE_LOGD("\n%s:\n", current_class);
			}

			if (j == 0)
				NOTE_LOGD("  %s", argument->name);
			else
				NOTE_LOGD(", %s", argument->name);

			if (argument->separator != '\0')
				NOTE_LOGD("%c*%s*", argument->separator, argument->value);
			else if (!detailed)
				NOTE_LOGD("\t");
		}
	}
...
void print_version(const Cli *cli)
{
	...
	NOTE_LOGD("built-in accelerators: process_vm = %s, seccomp_filter = %s\n",
...
static int parse_config(Tracee *tracee, size_t argc, char *const argv[])
{
	...
	if (get_care_cli != NULL) {
        ...
        NOTE_LOGD("extract_archive_from_file");
	...
	for (i = 1; i < argc; i++) {
		...
		options = cli->options;
		for (j = 0; options[j].className != NULL; j++) {
...
extern int initPRoot(int argc, char *const argv[])
{
    Tracee *tracee;
    int status;
    ...
    if (exit_failure) {
        NOTE_LOGD(stderr, "fatal error: see `%s --help`.\n", basename(argv[0]));
        exit(EXIT_FAILURE);
    }
    else
        exit(EXIT_SUCCESS);
