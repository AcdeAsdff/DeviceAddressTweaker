...
#include "../../libarchive-master/libarchive/archive.h"    /* archive_*(3), */
#include "../../libarchive-master/libarchive/archive_entry.h" /* archive_entry*(3), */
...
int extract_archive_from_file(const char *path)
{
	...
	if (status == ARCHIVE_WARN) {
        NOTE_LOGD("extract_archive_from_file[gzip]:%s",path);
	...
	if (status == ARCHIVE_WARN) {
        NOTE_LOGD("extract_archive_from_file[lzop]:%s",path);
		note(NULL, WARNING, INTERNAL, "add archive filter: %s",
			archive_error_string(archive));
	}
	else if (status != ARCHIVE_OK) {
