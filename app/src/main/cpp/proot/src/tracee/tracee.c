 ...
 */
static void clean_life_span_object(const void *pointer, int depth UNUSED,int max_depth UNUSED, int is_ref UNUSED, void* tracee_)
{
	Binding *binding;
	Tracee *tracee;
...
int new_child(Tracee *parent, word_t clone_flags)
{
	...
	child->heap = (Heap*)(
            ((clone_flags & CLONE_VM) != 0) ?
            talloc_reference(child,
                             parent->heap)
		: talloc_memdup(child,
                        parent->heap, sizeof(Heap)
                        )
        );
	if (child->heap == NULL)
		return -ENOMEM;
