...
#include <vector>
std::vector<dobby_instrument_callback_t> *g_supervisor_call_handlers;
...
static void common_supervisor_call_monitor_handler(DobbyRegisterContext *ctx, const InterceptEntry *info) {
  ...
  for (auto handler : *g_supervisor_call_handlers) {
    handler(ctx, (_RegisterContext *) info);
  }
}

void supervisor_call_monitor_register_handler(dobby_instrument_callback_t handler) {
  if (g_supervisor_call_handlers == NULL) {
    g_supervisor_call_handlers = new std::vector<dobby_instrument_callback_t>();
  }
...
void supervisor_call_monitor_register_svc(addr_t svc_addr) {
  ...
  DobbyInstrument((void *)svc_addr, (dobby_instrument_callback_t)common_supervisor_call_monitor_handler);
